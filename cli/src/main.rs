...
fn format_typescript_operation(output: &mut String, api: &zelyra_ast::ApiDef) {
    ...
    .expect("writing to a String cannot fail");
    output.push_str(
        "    if (!response.ok) throw new ZelyraApiError(response.status, await response.text());\n",
    );
...
mod tests {
    ...
    fn formats_typed_typescript_client_from_api_and_records() {
        let program = parse(
            &lex(
                "struct Address { city: String } api GET \"/customers/{id}\" { input { id: Id, term: String? } output Address } fn main() { }",
            )
            .unwrap(),
        )
        .unwrap();
