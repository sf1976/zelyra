    ...
    while i < bytes.len() {
        ...
        if c.is_ascii_alphabetic() || c == '_' {
            ...
            let kind = match word {
                ...
                "audit_chain" => TokenKind::AuditChain,
                ...
                "soft_delete" => TokenKind::SoftDelete,
                ...
                "confirm_page" => TokenKind::ConfirmPage,
                "success_page" => TokenKind::SuccessPage,
...
mod tests {
    ...
    fn lexes_auth_role_keywords() {
        let tokens = lex("auth users { roles: user_roles role_permissions: role_permissions audit: auth_audit_log audit_chain: true admin_path: \"/admin\" admin_permission: \"auth.manage\" admin_role: admin }").unwrap();
        assert!(tokens.iter().any(|token| token.kind == TokenKind::Roles));
        assert!(tokens
            .iter()
            .any(|token| token.kind == TokenKind::RolePermissions));
        assert!(tokens.iter().any(|token| token.kind == TokenKind::Audit));
        assert!(tokens
            .iter()
            .any(|token| token.kind == TokenKind::AuditChain));
        assert!(tokens
            .iter()
