...
Unveränderliche lokale Bindings werden ebenfalls durch den Funktionskörper
verfolgt. Sowohl `next: Int = value + 1` als auch die Kurzform
`next = value + 1` mit anschließendem `return next` können wie die direkte
Rückgabe zusammengefasst werden. Mutable Lokale und Zuweisungen werden in
diesem Schritt nicht zusammengefasst und bleiben `RUNTIME_CHECK`.
